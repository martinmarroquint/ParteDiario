...
const SelectorCPT = ({ onSelect, seleccionados = [], maxSelecciones = 10 }) => {
  ...
  const handleBusqueda = (valor) => {
    ...
    const filtrados = cptData.filter(([codigo, descripcion]) => 
    ...
    <div className="space-y-1.5">
        ...
        <div ref={dropdownRef} className="relative">
          <div className="relative">
            ...
            <input
              ...
              placeholder={`Buscar procedimiento CPT (${seleccionados.length}/${maxSelecciones})`}
      ...
      {seleccionados.length >= maxSelecciones && (
        <p className="text-[10px] text-amber-600 bg-amber-50 border border-amber-200 rounded-md px-2 py-1 text-center">
          Máximo {maxSelecciones} procedimientos alcanzados
        </p>
      )}
    </div>
  );

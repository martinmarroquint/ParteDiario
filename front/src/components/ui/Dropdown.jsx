...
const Dropdown = ({
  ...
  const handleInputFocus = () => {
    if (!disabled) {
      setIsOpen(true);
      if (searchable && inputRef.current) inputRef.current.select();
    }
  };
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`
            ...
            outline-none placeholder:text-gray-400 text-gray-900
            ${isReadOnly ? 'cursor-pointer select-none' : 'cursor-text'}
          `}
          style={{ caretColor: searchable ? '#0f766e' : 'transparent' }}
        />
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          {!disabled && showChevron && (
            <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-200 pointer-events-none flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
          )}
        </div>
      </div>
